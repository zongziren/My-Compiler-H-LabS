...
DECIMAL_FLOAT			[0-9]+(\.[0-9]+)?((E|e)(\+|-)?[0-9]+)?
HEX_FLOAT				"0"[xX][0-9a-fA-F]+(\.[0-9a-fA-F]+)?((P|p)(\+|-)?("0"[xX][0-9]+))?    
FloatConst				{HEX_FLOAT}|{DECIMAL_FLOAT}
...
{FloatConst}			{
							std::string floatconst = yytext;
							float sum = std::stof(floatconst); 
